...
exports.parse_addresses = function (raw_data) {
  if (!raw_data) {
    ...
    for (var line = 0, output_length = output.length - 1;
      ...
      var possible_link_index;
      var current_line_fields_index;
      if (isBsd) {
        possible_link_index = (new RegExp(/^[a-z].*[0-9]:$/)).test(current_line_fields[0]);
        current_line_fields_index = 0;
      } else {
        possible_link_index = parseInt(current_line_fields[0].split(':')[0]);
        current_line_fields_index = 1;
      }
      ...
      if (possible_link_index) {
         ...
         */
        // Update link pointer.
        current_link_name = current_line_fields[current_line_fields_index].split(':')[0];
